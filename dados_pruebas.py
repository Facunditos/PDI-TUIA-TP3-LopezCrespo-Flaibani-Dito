...
# 1. Leer el video y extraer frames.
...
frame_resized = cv2.resize(frame, dsize=(int(width / 2), int(height / 2)))

# 3. Convertir a escala de grises y resaltar color rojo.
img = cv2.cvtColor(frame_resized, cv2.COLOR_BGR2RGB)
img_hsv = cv2.cvtColor(img, cv2.COLOR_RGB2HSV)
h, s, v = cv2.split(img_hsv)
ix_h1 = np.logical_and(h > 179 * 0.9, h < 179 * 0.1)
ix_h2 = h < 180 * 0.04
...
# 4. Detectar contornos cuadrados.
def es_cuadrado(contorno):
...
frame_suavizado = cv2.GaussianBlur(img_gris, (5, 5), 0)  # Paso 2: Suavizar la imagen con un filtro gaussiano ( kernel 5 x 5)
_, umbral = cv2.threshold(frame_suavizado, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU) # Paso 3: Umbralización adaptativa con Otsu: Aplicar umbral adaptativo para resaltar contornos, requiere IMAGEN GRIS de entrada, convierte la imagen en binaria usando el método de Otsu
...
# 5. Recortar los contornos cuadrados detectados.
recortes = []
for contorno in contornos_cuadrados:
    mask = np.zeros_like(frame)
    cv2.drawContours(mask, [contorno], -1, 255, thickness=cv2.FILLED) # Dibujar en rojo con grosor de 2 píxeles
    dado_recortado = cv2.bitwise_and(frame_resized, frame_resized, mask=mask)
    recortes.append(dado_recortado)

# 6. Contar dados en las recortes.
...
for i, recorte in enumerate(recortes):
    ...
    cv2.putText(frame_resized, f'Puntos: {valor}', (x, y + h + 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)

# 7. Mostrar el resultado.
plt.imshow(frame_resized)
plt.show()
cv2.imwrite('frame_resultado.jpg', frame_resized)

# 8. Grabar un video procesado.
cap = cv2.VideoCapture(video_path)
fourcc = cv2.VideoWriter_fourcc(*'mp4v')
out = cv2.VideoWriter('video_procesado.mp4', fourcc, 20.0, (frame.shape[1], frame.shape[0]))
while True:
    ret, frame = cap.read()
    if not ret:
        break
    ...
    frame_procesado, _ = procesar_color(frame_resized)
...
############################## ANÁLISIS EXTRA COMPLEMENTARIO AL CÓDIGO #################################################################################
